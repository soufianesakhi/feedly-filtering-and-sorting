...
export class SettingsManager {
  ...
  importAllSettings(file: File) {
    ...
    fr.onload = () => {
      try {
        let settingsExport: SettingsExport = JSON.parse(fr.result as string);
        this.uiManager.autoLoadAllArticlesCB.refreshValue(
          settingsExport.autoLoadAllArticles
        );
        this.uiManager.loadByBatchEnabledCB.refreshValue(
          settingsExport.loadByBatchEnabled
        );
        this.uiManager.batchSizeInput.refreshValue(settingsExport.batchSize);
        this.uiManager.globalSettingsEnabledCB.refreshValue(
          settingsExport.globalSettingsEnabled
        );
  ...
  exportAllSettings() {
    this.dao.loadAll().then(subscriptions => {
      ...
      exportFile(
        JSON.stringify(settingsExport, null, 4),
        "feedly-filtering-and-sorting.json"
      );
    }, this);
  }
